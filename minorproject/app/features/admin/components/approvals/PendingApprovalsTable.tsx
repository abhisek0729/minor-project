...
export default function PendingApprovalsTable({
  ...
  const handleAction = (
    userId: number,
    roleName: "restaurantOwner" | "hotelOwner",
    ...
    startTransition(async () => {
      ...
      if (res.success) {
        toast.success(
          `${businessName} (${roleName === "restaurantOwner" ? "Restaurant" : "Hotel"}) has been ${action}!`
        );
      } else {
        // Rollback
        ...
        <Card className="border-dashed bg-muted/10 p-12 text-center">
          ...
          <p className="text-xs text-muted-foreground max-w-sm mx-auto mt-1">
            {search
              ...
              : "All partner registrations have been reviewed and verified. Good job!"}
          </p>
        </Card>
      ) : (
        <div className="grid gap-4 md:grid-cols-2">
          {filteredItems.map((item) => (
            <Card
              ...
              className="overflow-hidden border shadow-xs hover:border-primary/40 transition-all flex flex-col justify-between"
            >
              <div>
                ...
                <div className="relative h-44 w-full bg-muted overflow-hidden">
                  ...
                  {/* Title & Cuisine/Location on image */}
                  <div className="absolute bottom-3 left-3 right-3 text-white">
                    ...
                    <p className="text-xs text-white/80 line-clamp-1">
                      {item.cuisine ? `${item.cuisine} Cuisine` : item.businessLocation || "Nepal"}
                    </p>
                  </div>
                </div>
                  ...
                  <div className="space-y-1.5 pt-1 border-t">
                    <div className="flex items-center justify-between text-muted-foreground">
                      <span className="flex items-center gap-1.5">
                        <User className="size-3.5 text-primary" /> Owner Name:
                      </span>
                    ...
                    <div className="flex items-center justify-between text-muted-foreground">
                      <span className="flex items-center gap-1.5">
                        <Mail className="size-3.5 text-primary" /> Owner Email:
                      </span>
              ...
              <div className="p-5 pt-0 border-t flex items-center gap-3">
                <Button
                  ...
                  className="w-1/2 gap-1.5 text-xs font-semibold"
                >
                ...
                <Button
                  ...
                  className="w-1/2 gap-1.5 text-xs font-semibold bg-emerald-600 hover:bg-emerald-700 text-white shadow-xs"
                >
